...
class All:
    ...
    __slots__ = ("current_folder", "programs_folder", "proton_ge_aurl")
    ...
    def vkbasalt_all(self):
        ...
        subprocess.Popen(("make", f"{self.programs_folder}/tmp/vkBasalt")).wait()
        subprocess.Popen(
            ("make", f"{self.programs_folder}/tmp/vkBasalt", "install")
        ).wait()

        with open(
            os.path.expanduser("~/.local/share/vkBasalt/vkBasalt.conf"), "r+"
        ) as config_file:
    ...
    def gamemode_all(self, last_arg):
        ...
        api_link = (
            "https://api.github.com/repos/" + "FeralInteractive/Gamemode/releases"
        )
        dl_link = multi.get_release_data(api_link, "last")[0].get("download_url")
        ...
        subprocess.Popen(
            (f".{gamemode_path}/bootstrap.sh", last_arg),
    ...
    def create_install_script_all(self, create_icmd, _packages):
        ...
        if _packages:
            install_cmd = All.create_install_cmd_all(
                create_icmd[0], create_icmd[1], _packages
            )
...
class Arch(All, arch.Arch):
    ...
    def __init__(self, gpu_vendor):
        ...
        self._after = {"vkbasalt": [False, All], "gamemode": [False, All]}
    ...
    def install_script(self, install_programs):
        ...
        self.create_install_script_all([command, 2], packages)


class Fedora(All, fedora.Fedora):
    __slots__ = ("_top_commands", "_after", "gpu_vendor", "fedora_ver")
...
class Solus(All, solus.Solus):
    __slots__ = ("_top_commands", "_after")
...
class Ubuntu(All, ubuntu.Ubuntu):
    __slots__ = ("gpu_vendor", "_top_commands", "_after", "version")
    ...
    def lutris(self):
        self._top_commands.append("add-apt-repository ppa:lutris-team/lutris")
